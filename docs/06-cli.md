...
| `init <board> [dir]` | Copy MCU board scaffold (`main.kl`, Makefile, `klin.mod`; Arm/RP: `board/{startup.s,linker.ld,…}`; ESP: ESP-IDF `main/` + `sdkconfig.defaults`). Known: `nucleo-f411`, `pico`, `pico2`, `waveshare-rp2350-lcd-096`, `waveshare-esp32-s3-pico`. Default `[dir]` = `./<board>`. Host `klin init` (no ld) is out of scope — [075](../issues/075-board-pack-init-host.md) |
