...
**Status:** 🚧 — RP / ESP / STM8 / CH32V / GD32V via external `machine_*` (Pin…Adc); Wi‑Fi [`esp_wifi`](https://github.com/klin-lang/esp_wifi) ✅; ETH [`esp_eth`](https://github.com/klin-lang/esp_eth) ✅ (W5500); BLE [`esp_ble`](https://github.com/klin-lang/esp_ble) ✅; USB OTG [`esp_usb`](https://github.com/klin-lang/esp_usb) ✅ (device CDC); camera [`esp_camera`](https://github.com/klin-lang/esp_camera) ✅ (DVP JPEG); **ESP32-P4 Pin…Adc+Rmt** ✅ [`@v0.12.0`](https://github.com/klin-lang/machine_esp/releases/tag/v0.12.0) ([114](114-machine-esp-esp32-p4.md)); RMII / classic / freestanding later
...
| **ESP32-P4** | ✅ Pin…Adc+Rmt (`*_p4`) | Same package [`@v0.12.0`](https://github.com/klin-lang/machine_esp/releases/tag/v0.12.0) — `pin_out_p4`…`spi_out_p4` + `rmt_tx_p4` + `adc_out_p4` / `adc2_out_p4` ([114](114-machine-esp-esp32-p4.md)); MMIO `0x500E…` / RMT `0x500A2000` / LP_ADC `0x50127000` / HP_SYS_CLKRST (**not** a copy of C3/S3 `0x6000…`); LEDC **`freq_p4`**; ADC **`read_u12_p4`** (ADC1 CH0→GPIO16; ADC2 CH0→GPIO49); HP GPIO 0..54; **no** on-die Wi‑Fi/BLE (companion / other host). **On-chip EMAC** (RMII) → next [`esp_eth`](https://github.com/klin-lang/esp_eth) backend [102](102-esp-eth-idf.md) / [104](104-later-tracks-esp-network.md) E1. |
...
19. **ESP32-P4** — Pin…Adc+Rmt ✅ `machine_esp@v0.12.0` (`*_p4`; [114](114-machine-esp-esp32-p4.md); ADC2 ✅); [`esp_eth`](https://github.com/klin-lang/esp_eth) **RMII** backend later (on-chip EMAC; preferred first RMII target vs classic ESP32); Wi‑Fi/BLE only via companion / other host, not on-die; USB OTG can use [`esp_usb`](https://github.com/klin-lang/esp_usb) when P4 board work starts  
