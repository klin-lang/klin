...
2. **RCU** — `0x40023800`. GPIO clocks: `RCU_AHB1EN` `+0x30` bits `PAEN=0`,
   ...
   `SPIEN=12`. Default IRC16M **16 MHz** (not VF103 IRC8M).
...
9. **Core** — N307 (RV32IMAFDCPB). Examples stay polling; no ECLIC this tag.

Same `Pin` / `Pwm` / `Rc` / `Uart` / `Spi` methods as VF103 — only the factory
and register pointers differ. I2c is `I2cVw553` (new-style IP).
...
- Adc `*_vw553` (later tag, same package)
...
Do **not** call `pin_out` / `pwm_out` / `rc_out` / `uart_out` / `i2c_out` / `spi_out` (VF103) on a VW553.

```sh
klin get github/klin-lang/machine_gd32v@v0.7.0
```
